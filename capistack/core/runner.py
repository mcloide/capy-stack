"""
Deployment runner for CapiStack.
...
from capistack.core.logging import DeploymentLogger
from capistack.core.git import clone_repository, get_git_provider
...
class DeploymentRunner:
    ...
    def _load_configuration(self):
        """Load capistack.yml configuration if present."""
        config_path = self.repo_dir / 'capistack.yml'
        
        if config_path.exists():
            try:
                with open(config_path, 'r') as f:
                    self.config = yaml.safe_load(f) or {}
                self.logger.info("Loaded capistack.yml configuration", "checkout")
            except Exception as e:
                self.logger.warning(f"Failed to load capistack.yml: {e}", "checkout")
                self.config = {}
        else:
            self.logger.info("No capistack.yml found, using defaults", "checkout")
            self.config = {}
